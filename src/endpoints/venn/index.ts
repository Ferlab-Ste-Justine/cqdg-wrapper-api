import { buildQuery } from '@ferlab/next/lib/legacy';

import esClient from '#src/services/elasticsearch/client';
import { getESIndexByIndex, getGraphqlTypeByIndex } from '#src/utils';

import { Sqon, VennEntityOutput, VennOutput, VennOutputReformatted, VennOutputReformattedElement } from './types';
...
export const venn = async (sqons: Sqon[], index: string): Promise<VennOutput[]> => {
  ...
  const indexName = getESIndexByIndex(index);
  if (!mNestedFields.has(index)) {
    const nestedFields = getGraphqlTypeByIndex(index).extensions?.nestedFields || [];
    mNestedFields.set(indexName, nestedFields);
  }
  ...
  const r = await esClient.msearch({
    body: mSearchBody,
  });
